...
task :generate_hourly_flyability_scores => :environment do
  puts "event=rake_task_started task=flyability_scores:generate_all time=#{Time.zone.now.utc} UTC"
  FlySite.all.each do |fly_site|
    GenerateHourlyFlyabilityScores::call(fly_site)
  end
  puts "event=rake_task_finished task=flyability_scores:generate_all time=#{Time.zone.now.utc} UTC"
end

task :heroku_clean_up_old_data => :environment do
  cutoff_time = Time.zone.now.beginning_of_day.utc - 5.days
  puts "event=rake_task_started task=heroku_clean_up_old_data time=#{Time.zone.now.utc} UTC"
  ...
  puts "event=rake_task_finished task=heroku_clean_up_old_data time=#{Time.zone.now.utc} UTC"
end